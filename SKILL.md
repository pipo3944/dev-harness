...
description: >-
  ...
  引き継ぎは .dev/dev-harness/{issue}-{slug}/ の order.md / report.md / STATE.md に残り、再起動時は
...
- **ユーザーには平易な言葉で。** GATE・質問・方針確認では、運行内部の記号
  ...
  だけ（選択肢と背景を平易に添えて）。
- **引き継ぎはチャットでなくファイルで。** 計画・指示・報告は `.dev/dev-harness/`
  配下の `.md` に残す。誰でも作業ディレクトリを開けば現在地が分かる状態を保つ。
- **セッション分離は明示的に、黙ってやらない。** 重い工程は新しいコンテキストで
  ...
  - 小さい課題では層をまとめてよいが、必ずユーザー合意の上で。`.dev/dev-harness/`
    のファイル群がどの構成でも引き継ぎを成立させる。
...
1課題ぶんは `.dev/dev-harness/{issue#}-{slug}/` 配下（gitignored・個人用）。intake で
作成する。`.dev/` は他ツール・他用途とも共有しうる汎用領域なので、ハーネスは必ず
`dev-harness/` の名前空間の中に書く。`{slug}` は短い kebab-case
（例: `142-cart-total-rounding`）。
...
2. `.dev/dev-harness/{issue#}-{slug}/STATE.md` を探す。
...
- 恒久的な引き継ぎ物は `.dev/dev-harness/`（gitignored）へ。別セッション用の
  使い捨てプロンプトは `tmp/prompts/` へ。
